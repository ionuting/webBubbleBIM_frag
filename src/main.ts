...
// Mount nodal graph floating panel on body and wire the toolbar toggle
...
/**
 * Creates a minimal IFC spatial hierarchy (IfcProject → IfcSite →
 * IfcBuilding → IfcBuildingStorey) when no model is loaded yet.
 */
const ensureDefaultModel = async () => {
  if (fragments.list.size > 0) return;

  const modelId = "default-project";
  const bytes = FRAGS.EditUtils.newModel({ raw: true });
  await fragments.core.load(bytes, { modelId, raw: true });

  const editor = fragments.core.editor;

  /** IFC base-64 encoded 22-char GlobalId */
  const genGuid = (): string => {
    ...
    for (let i = 0; i < hex.length; i += 2) {
      ...
      bits += 8;
      while (bits >= 6) { bits -= 6; out += IFC64[(acc >> bits) & 0x3f]; }
    }
    if (bits > 0) out += IFC64[(acc << (6 - bits)) & 0x3f];
    return out.slice(0, 22);
  };

  const projectId = parseInt(editor.createItem(modelId, {
    category: "IFCPROJECT",
    guid: genGuid(),
    data: { Name: { value: "Default Project" } },
  }));

  const siteId = parseInt(editor.createItem(modelId, {
    category: "IFCSITE",
    guid: genGuid(),
    data: { Name: { value: "Default Site" } },
  }));

  const buildingId = parseInt(editor.createItem(modelId, {
    category: "IFCBUILDING",
    guid: genGuid(),
    data: { Name: { value: "Default Building" } },
  }));

  const storeyId = parseInt(editor.createItem(modelId, {
    category: "IFCBUILDINGSTOREY",
    guid: genGuid(),
    data: { Name: { value: "Ground Floor" } },
  }));

  await editor.relate(modelId, projectId, "IsDecomposedBy", [siteId]);
  await editor.relate(modelId, siteId, "IsDecomposedBy", [buildingId]);
  await editor.relate(modelId, buildingId, "IsDecomposedBy", [storeyId]);

  await fragments.core.update(true);
  console.log("[BIM] Default model created: Project → Site → Building → Ground Floor Storey");
};
